...
for (i in 0:periods) {
  ...
  newey <- lapply(mods_responses, function(x) {
...
plot(responses$bgs1,
  type = "l", main = "One-year rate",
  xlab = "", ylab = "",
  ylim = c(min(responses$low90bgs1), max(responses$up90bgs1)) * 1.1
)
polygon(
  x = c(seq_along(responses$bgs1), rev(seq_along(responses$bgs1))),
...
polygon(
  x = c(seq_along(responses$blip), rev(seq_along(responses$blip))),
...
plot(responses$bebp,
  type = "l", main = "Excess bond premium",
  xlab = "", ylab = "",
  ylim = c(min(responses$low90bebp), max(responses$up90bebp)) * 1.1
)
polygon(
  x = c(seq_along(responses$bebp), rev(seq_along(responses$bebp))),
...
plot(responses$blcpi,
  type = "l", main = "CPI",
  xlab = "", ylab = "",
  ylim = c(min(responses$low90blcpi), max(responses$up90blcpi)) * 1.1
)
polygon(
  x = c(seq_along(responses$blcpi), rev(seq_along(responses$blcpi))),
