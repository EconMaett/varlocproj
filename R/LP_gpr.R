...
for (i in 0:periods) {
  ...
  newey1 <- lapply(mods_responses, function(x) {coeftest(x, vcov. = NeweyWest(x, lag = (i+1), prewhite = FALSE))})
...
plot(responses$bip, type = "l", main = "IP", 
     xlab = '', ylab = '',
     ylim = c(min(responses$low90bip), max(responses$up90bip))*1.1)
polygon(x = c(seq_along(responses$bip), rev(seq_along(responses$bip))), 
...
plot(responses$bepu, type = "l", main = "EPU", 
     xlab = '', ylab = '',
     ylim = c(min(responses$low90bepu), max(responses$up90bepu))*1.1)
polygon(x = c(seq_along(responses$bepu), rev(seq_along(responses$bepu))), 
...
plot(responses$bsp500, type = "l", main = "S&P 500", 
     xlab = '', ylab = '',
     ylim = c(min(responses$low90bsp500), max(responses$up90bsp500))*1.1)
polygon(x = c(seq_along(responses$bsp500), rev(seq_along(responses$bsp500))), 
...
plot(responses$bgpr, type = "l", main = "GPR", 
     xlab = '', ylab = '',
     ylim = c(min(responses$low90bgpr), max(responses$bgpr))*1.1)
polygon(x = c(seq_along(responses$bgpr), rev(seq_along(responses$bgpr))), 
